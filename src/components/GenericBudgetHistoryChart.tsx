...
type GenericBudgetHistoryChartProps = {
  entityType: string;
  entityId: number;
};

export function GenericBudgetHistoryChart({
  entityType,
  entityId,
}: GenericBudgetHistoryChartProps) {
  const trpc = useTRPC();
  const authToken = useAuthStore((s) => s.accessToken) ?? "";

  const { data: raw, isLoading } = useQuery(
    trpc.getBudgetEntries.queryOptions({ authToken, entityType, entityId }),
  );
